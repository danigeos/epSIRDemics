...
dia_confinamiento2 <- dia_primer_dato + 24 #Lunes 30-April onset of confinement and alpha2
...
casos <- c(.083, #29 febrero
          ...
          131.646, 136.675, 141.942, 148.220, 153.222, 158.273 ) #from April 5th, data from https://www.worldometers.info/coronavirus/country/spain/
...
dead <- c(0.000, #29 febrero
          ...
          12.641, 13.341, 14.045, 14.792, 15.447, 16.081 ) #from April 5th, data from https://www.worldometers.info/coronavirus/country/spain/
...
q <- .012 #.17 proporción de infecciosos que son detectados. Vò study: 89 positives nearly none knew.
#valores iniciales [miles]:
S0 <- p*población 
I0 <- .05 #el 14-F había en España 2 casos confirmados
R0 <- 0
D0 <- 0
C0 <- I0*q
E0 <- 0
...
alpha1 <- 11.1e-6 #tasa capacidad de infeccion de Susceptibles`` [%/100/persona/dia]
alpha2 <- alpha1/1.3  #0e-6 #tras el confinamiento
alpha3 <- alpha2/1.9 #0e-6 #tras el cierre salvo esenciales
...
with(sird_values, {
  ...
  lines(time, C*q, col = "pink")
  abline(a=0,b=0,lty = 3, col="dark red")
  ...
  text(33,ymin,"2-A",cex=0.6,col="black")
  text(numero_datos+dia_primer_dato+1,ymin,"hoy",cex=0.6,col="black")
  with(datos,points(dias, casos, pch = 20, cex=0.5,col = "dark red"))
  with(datos,points(dias, dead, pch = 20, cex=0.5,col = "black"))
})

#Leyenda:
legend("topleft", c("Susceptibles","Infectious","Positivos acumulados","Symptomatic","Recovered","Dead", "Total"),
